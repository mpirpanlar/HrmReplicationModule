...
{
    public class BankReceiptReplicationTaskExtension : BoExtensionBase
    {
        ISystemService _createBulkUpdateTaskService;
        public BankReceiptReplicationTaskExtension(BusinessObjectBase bo)
            : base(bo)
        {
        }
        ...
        protected override void OnColumnChanged(object sender, DataColumnChangeEventArgs e)
        {
            ...
            // Detay tablosunda (Erp_BankReceiptItem) CurrentAccountId değiştiğinde
            if (e.Column != null && e.Column.ColumnName == "CurrentAccountId")
            {
                // Değişen satırın hangi tabloda olduğunu kontrol et
                if (e.Row != null && e.Row.Table != null && e.Row.Table.TableName == "Erp_BankReceiptItem")
                {
        ...
        protected override void OnAfterSucceededPost(object sender, EventArgs e)
        {
            ...
            // Evrak (banka fişi) tablosundaki alanlardan kontrol yap
            DataRow bankReceiptRow = BusinessObject.CurrentRow?.Row;
            if (bankReceiptRow == null)
                return;

            DateTime receiptDate;
            if (!DateTime.TryParse(bankReceiptRow["ReceiptDate"]?.ToString(), out receiptDate))
                isOk = false;
            ...
            if (isOk)
            {
                ...
                if (bankReceiptRow.Table.Columns.Contains("UD_IsTransactionReplicable") &&
                    bankReceiptRow["UD_IsTransactionReplicable"] != DBNull.Value)
                {
                    isReplicable = Convert.ToBoolean(bankReceiptRow["UD_IsTransactionReplicable"]);
                }
            ...
            if (isOk && bankReceiptRow.Table.Columns.Contains("UD_ReplicationStartDate"))
            {
                startDate = bankReceiptRow["UD_ReplicationStartDate"] == DBNull.Value
                    ? (DateTime?)null
                    : Convert.ToDateTime(bankReceiptRow["UD_ReplicationStartDate"]);

                if (startDate.HasValue && receiptDate < startDate.Value)
                    isOk = false;
            }

            if (isOk && bankReceiptRow.Table.Columns.Contains("UD_ReplicationEndDate"))
            {
                endDate = bankReceiptRow["UD_ReplicationEndDate"] == DBNull.Value
                    ? (DateTime?)null
                    : Convert.ToDateTime(bankReceiptRow["UD_ReplicationEndDate"]);
            ...
            if (isOk)
            {
                ...
                {
                    Convert.ToInt64(bankReceiptRow["RecId"].ToString())
                };
        ...
        private bool ValidateSingleCurrentAccount(CancelEventArgs e)
        {
            ...
            DataTable detailTable = BusinessObject.Data?.Tables["Erp_BankReceiptItem"];
            if (detailTable == null)
                return true;
            ...
            if (distinctCurrentAccountIds.Count > 1)
            {
                ...
                SysMng.Instance.ActWndMng.ShowMsg(
                    SLanguage.GetString("Banka fişinde birden fazla farklı cari hesap seçilemez. Lütfen tüm satırlarda aynı cari hesabı kullanın."),
                    ConstantStr.Warning,
        ...
        /// <summary>
        /// Cari hesap kartındaki replikasyon alanlarını detay satırına (Erp_BankReceiptItem) kopyalar
        /// </summary>
        ...
        /// <summary>
        /// Detay satırlarındaki replikasyon alanlarını başlığa (Erp_BankReceipt) kopyalar
        /// </summary>
